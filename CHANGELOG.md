...
- Deprecate explicitly passing `SolverXPBD(enable_restitution=False)`; XPBD will enable restitution by default in a future release. Omitting the argument preserves the legacy no-restitution default during the deprecation window. Pass `enable_restitution=True` to opt into the future behavior, and set material restitution coefficients to `0.0` for no-bounce contacts. (#1137)
